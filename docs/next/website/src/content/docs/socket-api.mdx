...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.send_mouse`, `pane.read`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Popup | `popup.close` |
...
`label` is the one presentation field that is always present in this payload:
it is `null` when the pane has no manual label, where the others are omitted
when unset. That is deliberate. A client must be able to tell a cleared label
(`"label": null`) from a server that predates the field (no `label` key at
all), and keep the label it already learned in the second case. `PaneInfo.label`
in `pane.get`, `session.snapshot`, and the `pane.rename` response is unchanged
and stays omitted when unset.
