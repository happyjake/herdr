...
| Area | Methods |
| --- | --- |
| Server | `ping`, `server.stop`, `server.reload_config`, `server.agent_manifests`, `server.reload_agent_manifests`, `server.lookup_place` |
...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.set_pinned`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.send_mouse`, `pane.read`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Popup | `popup.close` |
...
`server.lookup_place` turns a directory name into absolute directories this
server has already been to. It takes `query` and an optional `limit` (capped at
eight, and the default), and returns `places`: ranked `{ path, source }`
records for directories that still exist, where `source` is `workspace`, `z`,
`claude`, `tmux`, or `session`. The answer comes from memory alone — the
server's own live and persisted workspaces, the z frecency database, the Claude
Code project list, live tmux pane directories, and the working directories
recent agent sessions started in — and never from a disk search, so a directory
nobody has opened here is not a place it can name. Every source is optional and
a missing one is skipped silently.

Matching is case-insensitive. A folder-name match outranks a path-only match;
inside a tier a query the folder spells out outranks one that only survived a
typo (edit distance up to two on the folder name for queries of five or more
characters, one below that), and matches still level stay in source order. A
query starting with `/` or `~` is a literal path instead: it is answered as
itself when it exists, and otherwise its folder name is run through the matcher
so the near miss comes back as a candidate. A blank query or a `limit` of zero
is refused with `invalid_params`.

The lookup is bounded rather than best-effort: it is answered off the app
thread, tmux gets two seconds before it is skipped, the whole request gives up
after five and answers an empty list, and only two lookups run at a time per
server, so a client that asks faster than the server can answer queues instead
of piling on.

The method is additive: a server that predates it cannot parse the method and
refuses the request, which is how a client learns the lookup is absent.
...
`pane.send_mouse` sends a click or wheel action to one pane. `row` and `col`
are zero-based terminal cell coordinates and are clamped to the pane bounds.
The actions are `click`, `scroll_up`, and `scroll_down`; scroll actions accept
an optional `lines` count, with a missing or zero value treated as one. A click
is encoded as one atomic press-and-release PTY write when the pane has mouse
tracking enabled. Wheel actions follow the pane's live terminal state: mouse
reporting first, alternate-screen arrow emulation when DECSET 1007 is active,
and otherwise `host_scroll`. The API does not mutate the server's host viewport
for that last route.

```json
{"id":"req_click","method":"pane.send_mouse","params":{"pane_id":"w1:p1","action":"click","row":4,"col":12}}
{"id":"req_scroll","method":"pane.send_mouse","params":{"pane_id":"w1:p1","action":"scroll_up","row":4,"col":12,"lines":3}}
```

The response has type `pane_send_mouse`, a `delivered` boolean, and a `routing`
value of `mouse_report`, `alternate_scroll`, or `host_scroll`. Disabled mouse
tracking makes clicks dry (`delivered: false`); host-routed wheel actions are
also dry so a remote client can perform its own viewport scroll.
...
`PaneInfo.mouse_tracking` reports whether the live terminal currently accepts
mouse reports. Supporting servers always include this boolean. Clients reading
responses from older servers can treat an absent field as `false`.

`PaneInfo.alternate_screen` reports whether the live terminal currently uses
its alternate screen. Supporting servers always include this boolean. Clients
reading responses from older servers can treat an absent field as `false`.

`PaneInfo.agent_status_changed_at` reports the unix second at which the pane's
`agent_status` last changed, so a client that has just connected can show how
long a pane has held its status without having watched it change. A pane with
no detected agent reports `unknown` and dates it to the pane's own creation.
Supporting servers always include this field; clients reading responses from
older servers must treat an absent field as "age unknown" rather than as a
timestamp, and show no age at all.

The date is captured with the session, so it survives a server restart and a
live handoff: a pane that has been working for an hour still reads as an hour
old to a client that connects to the new process.

Getting that date back takes a moment. Until the new server has classified a
pane, what the pane reports is dated to the restart. A pane is classified by
the first of: the detector finishing its startup grace and saying what the
pane is doing, an agent reporting for itself — including a report that
confirms what the pane already showed — a deferred agent resume finishing or
failing, or any other change that moves what the pane reports. Changes that
say nothing about status, such as a title or a display label, never classify
a pane and never move its date. Once classified, a pane found doing what the session recorded gets
that older date back — but only if it reported no other known status in the
meantime. `unknown` does not count, because it is the absence of a reading
rather than a competing one: a client may well watch a pane go from `unknown`
to `working` and see the `working` dated to before the restart, and that is
correct, since the server never claimed the pane was doing something else. A
pane found doing anything else, or one that reported some other known status
on the way, is dated at the classification, because as far as this server
witnessed, that is when it started. A pane whose session recorded nothing running is classified the
moment it is rebuilt, and one that nothing classifies is classified on a
timer, so no pane stays undated. A session captured before this field existed
has no date to restore, and those panes date themselves to the restart.

A status the server watches change is always dated at the change, never back
at the restart. The guarantee this gives a client is precise: an age never
spans an observed transition between known statuses. Leaving `unknown` is not
such a transition.

The practical consequence for a client: for a few seconds after a server
restart, ages are short and then some of them jump back to their true values.
Ages during that window are never older than the restart, so a client that
sorts or filters on age will under-state rather than invent.

The two values are always read together, so the date describes exactly the
`agent_status` in the same payload and never some other status the pane held
earlier. A client can rely on that: if `agent_status` is `unknown`, the date
is when the pane started reporting `unknown`.

Every change of the reported status restamps the field, including a `done` to
`idle` change caused by the pane being seen at the desk rather than by the
agent doing anything. A client that deliberately folds read-driven transitions
without ageing its own rows will therefore disagree with a freshly seeded one:
an age seeded from `agent_status_changed_at` is at most as old as the last desk
read of that pane, while a client that watched the same transition still holds
the older stamp. Both are honest about what they witnessed, so a client that
folds read-transitions should expect the seeded age to be the shorter of the
two and not treat the difference as drift.
...
The `ping` response also carries the server's version, protocol, capabilities, and its declared display name (`websocket_api.name`, defaulting to the machine hostname) — identical over the Unix socket and the WebSocket transport. It may also carry `reach`, the operator-declared route to the server; `advertised_endpoint`, the URL the server declares clients should dial to reach it ([`websocket_api.advertised_endpoint`](/docs/configuration/#websocket-api-listener)), in the same canonical form the pairing payload carries; `session`, the live non-default Herdr session name; and `exe`, the absolute path of the running executable. `reach` is omitted when unset or empty, `advertised_endpoint` is omitted when nothing dialable is declared, and `session` is omitted for the default session. These fields are additive: clients talking to older servers must treat them as optional.

Event subscriptions keep the connection open after the initial response.

## WebSocket transport

The same JSON API can optionally be served over WebSocket for non-terminal clients. The listener is a transport adapter in front of the same request dispatch and event subscription paths, so requests, responses, and event payloads are identical to the Unix socket. It is off by default and requires explicit configuration; see [Configuration](/docs/configuration/#websocket-api-listener).

Framing differences from the Unix socket:

- Send one JSON message per text frame instead of one JSON line.
- A connection may carry sequential requests: after a response you can send the next request on the same connection. (The Unix socket serves one request per connection.)
- `events.subscribe` streams until the client disconnects, but it does not reserve the connection: requests sent while the stream is running are served in place and the stream keeps delivering events. (On the Unix socket, writing anything after starting a stream ends the connection.)
- One stream at a time per connection. A second streaming request — `events.subscribe`, `events.wait`, `agent.prompt`, `agent.wait`, or `pane.wait_for_output` — sent while a stream is running is refused with `stream_busy`; the running stream is unaffected. Open a second connection for a second stream. (`pane.graphics.stream` still returns `unsupported_transport` over WebSocket, stream or no stream.)

Clients can detect this without probing: the `ping` response advertises `capabilities.stream_multiplex`. It describes the server, not one connection — the pong payload is identical on every transport — so read it together with the transport you are on: WebSocket honors it, the Unix socket cannot multiplex.

Authentication happens at the WebSocket handshake with the configured bearer token. Send an `Authorization: Bearer <token>` header, or a `token` query parameter from clients that cannot set headers:

```text
websocat -H "Authorization: Bearer $TOKEN" ws://100.64.0.5:4433
```

```js
// Browser: headers are not settable on WebSocket upgrades.
const ws = new WebSocket("ws://100.64.0.5:4433/?token=" + token);
```

Handshakes with a missing or invalid token are rejected with HTTP 401 before any API processing.

`herdr pair` provisions the token: it mints a fresh one, stores it in config.toml, applies it to a running server, and prints the endpoint + token as a QR code and plaintext for client setup. Re-running it rotates the token and invalidates the previous one. See the [CLI reference](/docs/cli-reference/#pairing).
...
`pane.set_pinned` sets or clears a pane's pin and answers `pane_info`, the way
`pane.rename` does, so the client that toggled it can fold the answer instead
of waiting for its next snapshot:

```json
{"id":"req_pin","method":"pane.set_pinned","params":{"pane_id":"w1:p1","pinned":true}}
```

The pin is a durable per-pane mark. It is kept, never derived: it is set only
by this method or by `herdr pane pin` / `herdr pane unpin`, it persists with the
session across a restart, and it says nothing about the pane's label, agent, or
status. `PaneInfo.pinned` is always present and `true` or `false`, unlike the
optional presentation fields, so a client can read an absent `pinned` key as a
server that predates the pin and offer no pinning at all.

The pin is the user's own mark. Agents may read `pinned` freely and should set
it only when the user explicitly asks for it, never on their own initiative.
This is a convention where agents are taught, not a rule the server enforces.
...
`pane.agent_status_changed` is scoped to one `pane_id` and emits `pane_id`,
`workspace_id`, `agent_status`, and the pane's presentation: optional `agent`,
`title`, `display_agent`, `state_labels`, and `label`. `label` is the pane's
manual label, the one `pane.rename` sets and the pane menu edits. Changing it
publishes the event on its own, with `agent_status` reporting whatever the pane
already held, so a subscribed client sees a rename as it happens rather than at
its next snapshot.

The payload also carries `pinned`, the pane's pin. Changing it publishes the
event on its own in the same way a label change does, with `agent_status`
reporting whatever the pane already held.

`label` and `pinned` are the presentation fields always present in this
payload: `label` is `null` when the pane has no manual label and `pinned` is
`true` or `false`, where the others are omitted when unset. That is deliberate.
A client must be able to tell a cleared label (`"label": null`) or an unpinned
pane (`"pinned": false`) from a server that predates the field (no key at all),
and keep what it already learned in the second case. `PaneInfo.label` in
`pane.get`, `session.snapshot`, and the `pane.rename` response is unchanged and
stays omitted when unset; `PaneInfo.pinned` is always present there too.
...
A server honors a registry of credentials rather than a single token: one
**managing** credential — the token `herdr pair` mints into
`[websocket_api].token`, rotated by re-pairing — plus any **limited**
credentials the managing credential has minted, for example one per linked
browser. Every live credential authenticates the WebSocket handshake and
drives the whole API; the tier decides only who may manage the registry.

The registry is runtime state stored beside the session, not configuration.
Minting never edits `config.toml`, a restart keeps every credential, and
re-pairing rotates the managing credential alone — limited credentials
survive it. Credentials are stored as fingerprints: a minted token is
returned once, at mint time, and cannot be read back.

Servers that support this declare `credential_registry: true` in the `ping`
capabilities. Older servers cannot parse the methods and answer
`invalid_request`; the protocol version is unchanged.

| Method | Who may call it | Params |
| --- | --- | --- |
| `credential.mint` | managing only | `label` (optional) |
| `credential.list` | managing only | — |
| `credential.revoke` | managing, or any credential on itself | `credential_id` (optional; absent means the caller) |
| `credential.revoke_all` | managing only | — |

```json
{ "id": "req_1", "method": "credential.mint", "params": { "label": "desk browser" } }
```
...
```json
{
  ...
  "result": {
    ...
    "credential": {
      "credential_id": "cred_7f3a1c0d9b2e4a61",
      "tier": "limited",
      "label": "desk browser",
      "created_at_unix": 1781043522
    },
    "token": "0Zx3…"
  }
}
...
`credential.list` returns `credential_list` with one `credential` object per
live credential, managing first, each carrying `credential_id`, `tier`,
`label`, `created_at_unix`, and `last_seen_unix` once the credential has been
used. Tokens are never listed. `credential.revoke` and `credential.revoke_all`
both return `credential_revoked` with the `revoked` array of ended
`credential_id`s.

Over the Unix socket there is no handshake, so the caller acts as the
managing credential by default — owning the socket file is already full
authority over the server. A local caller may act as a specific credential by
passing its token as `acting_token`; on an authenticated transport the
credential presented at the handshake is the actor and `acting_token` is
refused.

### Refusals a credential client must branch on

Three codes are verdicts about the caller's credential. Everything else —
`internal_error`, `server_unavailable`, `invalid_params`, a dropped
connection — is trouble: retry, and change nothing stored locally.

| Code | Meaning | What a client should do |
| --- | --- | --- |
| `credential_revoked` | The credential is gone from this server: revoked, or never issued here. | Discard it and pair or link again. Retrying never succeeds. |
| `credential_forbidden` | Live credential, wrong tier: a limited credential may not mint, list, or revoke another. | Nothing; the credential still works for everything else. |
| `credential_not_found` | No credential with that `credential_id` is in the registry. | Refresh the list; the caller's own credential is unaffected. |

`credential_revoked` is deliverable on an established connection, because a
browser cannot read a rejected handshake's HTTP status — a 401 and a dead
endpoint are the same opaque close. A credential the server revoked is
therefore admitted at the handshake and answered with `credential_revoked`,
and a connection whose credential is revoked while it is open is answered the
same way — including one holding an `events.subscribe` stream, which receives
the refusal with an empty response `id` before the socket closes. The verdict
is terminal: nothing else is served on that connection. A token the server
never issued still gets the plain handshake rejection.
