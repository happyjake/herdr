...
The `ping` response also carries the server's version, protocol, capabilities, and its declared display name (`websocket_api.name`, defaulting to the machine hostname) — identical over the Unix socket and the WebSocket transport. The `name` field is additive: clients talking to older servers must treat it as optional.
