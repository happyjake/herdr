...
`pair` mints a fresh bearer token for the [WebSocket API listener](/docs/socket-api/#websocket-transport), stores it as `[websocket_api].token` in config.toml, and prints the pairing payload — the endpoint URL plus token — as a terminal QR code and as plaintext. The QR encodes the directly connectable form `ws://<bind>/?token=<token>&name=<name>`, where `name` is the server's percent-encoded display name (`[websocket_api].name`, or the machine's hostname when unset) so a scanning client can label the server before first connect. The plaintext endpoint stays bare — the name and token ride only the scannable URL. When [`[websocket_api].advertised_endpoint`](/docs/configuration/#websocket-api-listener) is set, the payload names that URL instead of the bind address, so a listener behind a TLS terminating proxy pairs as `wss://<host>/?token=<token>&name=<name>`, or as `wss://<host>/<path>?token=<token>&name=<name>` when the declared URL carries a path, which is the same path the server publishes in its pong; an advertised endpoint that could not be dialed is refused instead of encoded. A running server is asked to reload its config so the new token takes effect without a restart. That request is an attempt, not the result: what `pair` reports comes from using the token, and every message states both — what the reload attempt did, and what the handshake found. It dials the url it just printed — host, port and path as written — presents the token the way the payload does, and requires the peer to answer as herdr before calling it live, because a websocket upgrade identifies a service and not this server. Re-running the command rotates the token, and the report says the previous one has stopped authenticating only when presenting it to that same peer is refused; a peer that never identified itself is never shown the previous token at all. A refused handshake is reported as exactly that and no more: herdr answers an unknown token with a deliberately opaque `401`, indistinguishable from any other service's and from a proxy that rejects before forwarding, so a refusal says the connection did not get in — never which process refused, which config it holds, or that a reload will change it. A `wss://` payload is the one url this build cannot dial, having no TLS client: there the proof is attempted against the listener behind the proxy, the message names the printed url as never dialed, and no outcome promises that payload works. Every other outcome is named for what it is — no listener, a peer that never answered as herdr, or an exchange that settled neither way — and `pair` exits non-zero when the payload was observed not to work or could not be shown to. Resolution, connection, the handshake exchange and each control-socket attempt are bounded, so a stalled proxy or a stale server ends in a named outcome instead of a hang. It requires `[websocket_api].bind` to be configured and explains what to enable when it is not.
...
herdr pane pin <pane_id>
herdr pane unpin <pane_id>
...
`pane pin` and `pane unpin` set the pane's pin, the durable mark a user puts
on a pane. It survives a restart, and `pane list` and `pane get` report it as
`pinned`. The pin is the user's own mark: read it freely, and set it only when
