...
`pair` mints a fresh bearer token for the [WebSocket API listener](/docs/socket-api/#websocket-transport), stores it as `[websocket_api].token` in config.toml, and prints the pairing payload — the endpoint URL plus token — as a terminal QR code and as plaintext. The QR encodes the directly connectable form `ws://<bind>/?token=<token>`. A running server picks the new token up immediately through a config reload. Re-running the command rotates the token: the previous one stops authenticating. It requires `[websocket_api].bind` to be configured and explains what to enable when it is not.
