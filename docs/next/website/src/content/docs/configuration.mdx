...
It must be a `ws://` or `wss://` URL carrying scheme, host, and optional port and nothing else: `herdr pair` refuses to print a payload it cannot make connectable, naming what is wrong, rather than encoding a QR nobody can scan into a connection. Unset or empty keeps the bind-derived `ws://<bind>` URL exactly as before.
