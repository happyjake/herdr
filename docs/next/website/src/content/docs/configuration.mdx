...
The transport is plain `ws://`; the network layer (for example your tailnet's WireGuard encryption) provides transport security. Changes to this section require a server restart — `server.reload_config` does not rebind the listener. See [Socket API](/docs/socket-api/) for the wire protocol.
