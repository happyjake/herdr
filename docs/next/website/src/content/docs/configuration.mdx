...
It is also published in the `ping` pong as `advertised_endpoint`, so a client that is already paired learns the URL to dial without pairing again; a server that declares nothing dialable publishes no field. It must be a `ws://` or `wss://` URL carrying scheme, host, and optional port and nothing else. `herdr pair` validates it before minting anything, and accepts only what a client provably dials as exactly the host and port written: a host name, a bracketed IPv6 literal, or an IPv4 address as four plain decimal octets. That is deliberately narrower than everything a browser tolerates — `0x7f000001`, `010.0.0.1`, `127.1` and `192.0.2.1.` are all refused, because a client dials each of them as a different address than it spells, and a port is refused unless it is plain digits (`:+443` and `:0443` are not). A non-ASCII host must be given in its punycode (`xn--`) form. Every refusal names the canonical form to write instead, rather than encoding a QR nobody can scan into a connection. Unset or empty keeps the bind-derived `ws://<bind>` URL exactly as before.
