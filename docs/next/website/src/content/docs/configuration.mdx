...
## WebSocket API listener

The JSON API can optionally be served over WebSocket for non-terminal clients. The listener is off by default: with no `bind` address, no network port is opened, and nothing enables it implicitly.

```toml
[websocket_api]
bind = "100.64.0.5:4433"
token = "minted-by-herdr-pair"
name = "The Mini"
reach = "the-mini"
```

`bind` is the exact TCP address the listener binds; prefer a tailnet or loopback address. `token` is required whenever `bind` is set — the server refuses to start without it — and clients must present it during the WebSocket handshake, either as an `Authorization: Bearer <token>` header or as a `token` query parameter for clients that cannot set headers (browsers). Tokens may use ASCII letters, digits, and `-._~` only, so the query form never needs URL escaping. Handshakes with a missing or wrong token are rejected with HTTP 401 before any API request is processed.

`name` is the optional display name the server declares about itself: it is returned in the `ping` response on every transport and embedded in the pairing QR payload so clients can show a friendly label. Unset or empty means the machine's hostname. Names are display only, never identity — clients tell servers apart by endpoint.

`reach` is the optional route another machine uses to open a shell on this server, typically an exact SSH alias. It is kept separate from `name` because display styling and a working route can differ. Herdr never guesses reach from the display name or hostname; unset or empty means undeclared.

`advertised_endpoint` is the optional URL clients should dial to reach this listener. The bind address describes the local socket, which is the reachable URL only when nothing fronts it; declare an advertised endpoint when something does — a TLS terminating proxy, for instance — so the pairing payload names the URL that works:
...
It is also published in the `ping` pong as `advertised_endpoint`, so a client that is already paired learns the URL to dial without pairing again; a server that declares nothing dialable publishes no field. It must be a `ws://` or `wss://` URL carrying scheme, host, optional port, and optional path and nothing else. `herdr pair` validates it before minting anything, and accepts only what a client provably dials as exactly the host, port, and path written: a host name, a bracketed IPv6 literal, or an IPv4 address as four plain decimal octets. That is deliberately narrower than everything a browser tolerates — `0x7f000001`, `010.0.0.1`, `127.1` and `192.0.2.1.` are all refused, because a client dials each of them as a different address than it spells, and a port is refused unless it is plain digits (`:+443` and `:0443` are not). A non-ASCII host must be given in its punycode (`xn--`) form. Every refusal names the canonical form to write instead, rather than encoding a QR nobody can scan into a connection.

A path is how one fronting proxy serves several Herdr servers on one port, one path each:

```toml
[websocket_api]
advertised_endpoint = "wss://a-host.example.net/herdr-ws"
```

The pairing payload puts its query on after the declared URL, so that endpoint is dialed as `wss://a-host.example.net/herdr-ws?token=<token>&name=<name>` and read back by a client as that host and that path — the same ones the pong publishes, so the QR and the pong name one server rather than two. A trailing slash is dropped from the declared form, so `…/herdr-ws/` and `…/herdr-ws` pair identically. An endpoint that declares no path keeps the URL it has always had, `wss://a-host.example.net:8443/?token=<token>&name=<name>`, which is what every already-paired server is keyed by. Paths are held to the same standard as hosts: only ASCII letters, digits, hyphens, dots, underscores, and tildes, in non-empty segments separated by single slashes, with no percent escapes and no `.` or `..` segments — a client resolves those away before dialing, so the path it dials would not be the path declared. A query, a fragment, and credentials stay refused: those collide with the token the payload appends instead of surviving in front of it. Unset or empty keeps the bind-derived `ws://<bind>` URL exactly as before.

Declaring an advertised endpoint also makes a wildcard `bind` usable: `0.0.0.0:4433` is the ordinary listener shape behind a proxy, and while it is refused on its own (no client can dial it), with an advertised endpoint nothing derives the payload from it any more. `herdr pair` still installs the freshly minted token on the running server, reaching a wildcard listener on loopback at the same port. A `bind` port of `0` stays an error either way, since nothing can know which port the listener ends up on.

Set `bind` by hand, then let [`herdr pair`](/docs/cli-reference/#pairing) manage the token: it mints and stores a fresh token, applies it to a running server, and prints the pairing payload as a QR code and plaintext. Re-running it rotates the token and invalidates the previous one.

The listener itself speaks plain `ws://`; the network layer (for example your tailnet's WireGuard encryption) provides transport security, and a proxy in front of it can add TLS. Changing `bind` requires a server restart — `server.reload_config` does not rebind the listener. A changed `token`, `name`, or `reach` is applied by `server.reload_config` without a restart; connections authorized before the change stay connected. A changed `advertised_endpoint` needs no restart either — `server.reload_config` republishes it in the pong, and `herdr pair` reads it from the config file on every run. See [Socket API](/docs/socket-api/) for the wire protocol.
...
While no client is attached, the server lays panes out at least this large (default 180×60), so API readers such as the mobile client see agent output at real-terminal proportions. An attached client's real terminal size always wins while it is connected, and a larger last-attached size is kept after detach. Values below the 80×24 minimum are raised to it:

```toml
[advanced]
headless_min_cols = 180
headless_min_rows = 60
```
