...
The transport is plain `ws://`; the network layer (for example your tailnet's WireGuard encryption) provides transport security. Changing `bind` requires a server restart — `server.reload_config` does not rebind the listener. A changed `token`, `name`, or `reach` is applied by `server.reload_config` without a restart; connections authorized before the change stay connected. See [Socket API](/docs/socket-api/) for the wire protocol.
