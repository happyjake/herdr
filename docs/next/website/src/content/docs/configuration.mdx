...
It must be a `ws://` or `wss://` URL carrying scheme, host, and optional port and nothing else, with a host a URL parser reads the same way Herdr does — an IPv4 literal, a bracketed IPv6 literal, or a host name. `herdr pair` validates it before minting anything and refuses to print a payload it cannot make connectable, naming what is wrong, rather than encoding a QR nobody can scan into a connection. Unset or empty keeps the bind-derived `ws://<bind>` URL exactly as before.
